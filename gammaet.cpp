...
#include <QThread>
#ifdef GAMMAET_H
GammaET::GammaET(QObject *parent , QString *ip, quint16 *port) : QObject (parent)
{
...
GammaET::GammaET(QObject *parent , QString *ip, quint16 *port, int type) : QObject (parent)
{
    m_sock = new QTcpSocket(this);
    connect(m_sock, SIGNAL(readyRead()), this, SLOT(readData()));
    // connect(m_sock, SIGNAL(bytesWritten(qint64)), this, SLOT(writes()));
...
void GammaET::changeInterface(const QString &address, quint16 portNbr)
{
    m_sock->connectToHost(address, portNbr);
}
void GammaET::on_cbEnabled_clicked(bool checked)
{
    if (checked)
    {
    }
...
void GammaET::readData()
{
    ...
    qDebug() << "Modbus GammaET measure equipment data: " << data << " lenght - " << data.length() << " \n\r";
    ...
    while (i != -1)
    {
        ...
        if (_tmp.toInt() == 0) //error detecting if modbus function
        {
            ...
            pool = _tmp.toInt()*2; //quantity of retreived nibbles = bytes*2
            if (pool > 0){
                //i++;// first element 7
                ...
                int _type = _tmp.toInt(nullptr,16);

                if (_type == 1){ name = "NH3"; //detect type of a sensor HARDCODED for GammaET equipments
                }
                if (_type == 2){ name = "CS2"; //detect type of a sensor HARDCODED for GammaET equipments
                }
                if (_type == 3){ name = "H2S"; //detect type of a sensor HARDCODED for GammaET equipments
                }
                if (_type == 4){ name = "NO2"; //detect type of a sensor HARDCODED for GammaET equipments
                }
                if (_type == 5){ name = "CH"; //detect type of a sensor HARDCODED for GammaET equipments
                }
                if (_type == 6){ name = "SO2"; //detect type of a sensor HARDCODED for GammaET equipments
                }
                if (_type == 7){ name = "NO+NO2"; //detect type of a sensor HARDCODED for GammaET equipments
                }
                if (_type == 8){ name = "NO"; //detect type of a sensor HARDCODED for GammaET equipments
                }
                if (_type == 9){ name = "CH4"; //detect type of a sensor HARDCODED for GammaET equipments
                }
                if (_type == 10){ name = "HCH"; //detect type of a sensor HARDCODED for GammaET equipments
                }
...
void GammaET::sendData(int address, int registers)
{
    /*  int checksum =  0; //id = 0

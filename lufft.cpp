...
#ifdef LUFFT_H
Lufft::Lufft(QObject *parent , QString *ip, quint16 *port) : QObject (parent)
...
Lufft::Lufft(QObject *parent , QString *ip, quint16 *port, float _in, float _out) : QObject (parent)
...
void Lufft::changeInterface(const QString &address, quint16 portNbr)
{
...
void Lufft::on_cbEnabled_clicked(bool checked)
{
    if (checked)
    {
    }
...
void Lufft::displayError(QAbstractSocket::SocketError socketError)
{
    switch (socketError) {
...
float Lufft::compare(float _in, float _prev)
{
    if (!first_run ){
